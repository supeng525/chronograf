...
Dygraph.prototype.xTradingHours1 = function () {
  ...
  // console.log('sup',tradingHours1,tradingHours1[1].getHours())
  // return this.dateWindow_ ? this.dateWindow_ : this.getStringOption("tradingHours1");
  // 返回当天日期  注意  上午12点为0点  考虑20点后为夜盘交易时间
  // if(tradingHours1[1]<tradingHours1[0] && tradingHours1[0] < 19)
  // {
...
Dygraph.prototype.xAxisExtremes = function () {
  ...
  if (pad) {
   ...
   var endInterval = 0;
   // tradingHours1非空
   if(tradingHours1[2]!=0 && tradingHours1[3]!=0){
     // tradingHours2也非空
     if(tradingHours2[2]!=0 && tradingHours2[3] !=0){
        startInterval = ((new Date(left).getHours()*60 + new Date(left).getMinutes())-tradingHours1[2])*60*1000
        endInterval = ((new Date(right).getHours()*60 + new Date(right).getMinutes())-tradingHours2[3])*60*1000
     }else if(tradingHours1[2] <=tradingHours1[3]){//非夜盘
        startInterval = ((new Date(left).getHours()*60 + new Date(left).getMinutes())-tradingHours1[2])*60*1000
        endInterval = ((new Date(right).getHours()*60 + new Date(right).getMinutes())-tradingHours1[2])*60*1000
     }else if(tradingHours1[2] > tradingHours1[3]){//夜盘
        var leftTime = new Date(left).getHours()*60 + new Date(left).getMinutes();
        var rightTime = new Date(right).getHours()*60 + new Date(right).getMinutes();
        if(leftTime > tradingHours1[3] && leftTime < tradingHours1[2])//最小时间介于停盘时间
        {
          startInterval = (leftTime-tradingHours1[2])*60*1000;
        }
        if(rightTime > tradingHours1[3] && rightTime < tradingHours1[2])
        {
          endInterval = (rightTime-tradingHours1[3])*60*1000;
        }
     }
   }
   if(startInterval<0)
...
Dygraph.prototype.getScale = function () {
  ...
  // 标记交易时段分类类型  0：为不设置交易时段 1：tradingHours1和2非空 day+1 2:day-1 3:day不变 4：tradingHours1非空  5：夜盘
  var type =6;//tradingHours1和2均为空
  // 先考虑 tradingHours1不为空的情况
  if(tradingHours1[2]!=0 && tradingHours1[3]!=0){
   //  tradingHours1和2都不为空的情况
   type =0;
   if(tradingHours2[2]!=0 && tradingHours2[3] !=0)
   {
     // 中午休市时间
     ...
     // 最大时间 > 中午休市后开盘时间 && 最小时间 <= 中午休市开始时间
     if(limitEnd >= tradingHours2[2] && limitStart <= tradingHours1[3]){
       ...
       // tradingHours1和2均非空 且day不变
       type = 3;
     }
     //只有tradingHours1的情况下，且非夜盘
   }else if(tradingHours1[2] <= tradingHours1[3])
   {
     breaktime = 0; //中午休市时间为0
     // 休市到第二日
     stoptime = 24*60*60*1000 - (tradingHours1[1].getTime() - tradingHours1[0].getTime());
     scale = scale - stoptime*dayAll;
   // tradingHours1非空
     type = 4;
   }else if(tradingHours1[2] > tradingHours1[3]) {//夜盘
     breaktime = 0; //中午休市时间为0
     // 休市到第二日
     stoptime = tradingHours1[0].getTime() - tradingHours1[1].getTime();
     // xrange = xrange - stoptime*day;
     // 待确认
     if(dayAll>=2){
       // 最大时间 >=夜盘开始时间
       if(limitEnd >= tradingHours2[2])
       {
        scale = scale - stoptime*(dayAll-1);
       }else{
        scale = scale - stoptime*(dayAll-2);
       }
     }
     // 夜盘
     type = 5;
   ...
   var data = {'scale':scale,'type':type,'day':dayAll,'breaktime':breaktime,'stoptime':stoptime};
   return data;
}
...
Dygraph.prototype.transformX = function (x)
{
  ...
  switch(type){
   ...
   case 3://tradingHours1和2非空 day不变
       ...
       break;
   case 4: //tradingHours1非空
       temp = x -  stoptime*day;
       break; 
   case 5: // tradingHours1非空 夜盘
       if(day>=2){
       // 最大时间 >=夜盘开始时间
         if(timeVal >= tradingHours2[2])
         {
           temp = x - stoptime*(day-1);
         }else{
           temp = x - stoptime*(day-2);
         }
       }
       break;
   default:
...
//       // 休市时间
//       var breaktime = 0
//       // trading Hours1和2的开始时间小于结束时间
//       if(tradingHours2[0] <= tradingHours2[1] && tradingHours1[0] <= tradingHours1[1])
//       {
//         // 休市时间
//         breaktime = tradingHours2[0].getTime() - tradingHours1[1].getTime()
//         if(breaktime <= 0){
...
//       }
//       // 闭市时间 到第二日
//       var stoptime = 24*60*60*1000 - (tradingHours2[1].getTime() - tradingHours1[0].getTime());
//       if(stoptime <= 0)
//       {
//         stoptime = 0;
//       }
...
//       var limitStart = new Date(xRange[0]).getHours()*60 + new Date(xRange[0]).getMinutes();
//       var limitEnd = new Date(xRange[1]).getHours()*60 + new Date(xRange[1]).getMinutes();
//       // 最小时间按分钟计算  大于  最大时间  天数+1 
//       if(limitStart > limitEnd || (xRange[1] - xRange[0])/24/60/60/1000 -day >0.9999) 
//       {
//         day = day +1;
//         dayAll = dayAll + 1;
//       }
...
//       // 大于中午闭市后时间
//       if(timeVal >= tradingHours2[2]){
//         pct = (x - xRange[0] - breaktime*(day+1) - stoptime*day)/scale
//         //pct = (x - xRange[0] - temp)/(xRange[1]- xRange[0] - temp)
//       }else if(timeVal<=tradingHours1[3]){//介于tradingHours1 开始和结束之间的时间
//         pct = (x - xRange[0] -(breaktime + stoptime)*day)/scale 
//       }
//       else{
//         pct = (x - xRange[0])/scale
//       }
//     }
//     }else{
