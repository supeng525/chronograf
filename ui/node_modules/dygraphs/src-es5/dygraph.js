...
Dygraph.prototype.xTradingHours2 = function () {
  ...
  // 考虑19点后为夜盘时间
  if(tradingHours2[1]<tradingHours2[0] && tradingHours2[0] < 19)
  {
...
Dygraph.prototype.xAxisExtremes = function () {
  ...
  if (pad) {
   ...
   if(tradingHours1[2]!=0 && tradingHours1[3]!=0 && tradingHours2[2]!=0 && tradingHours2[3] !=0){
    startInterval = ((new Date(left).getHours()*60 + new Date(left).getMinutes())-
    (tradingHours1[0].getHours()*60 + tradingHours1[0].getMinutes()))*60*1000
    endInterval = ((new Date(right).getHours()*60 + new Date(right).getMinutes())-
    (tradingHours2[1].getHours()*60 + tradingHours2[1].getMinutes()))*60*1000
   }
   if(startInterval<0)
   {
     left = left - startInterval;
   }
   if(endInterval > 0 && tradingHours2[3]!=0)//tradingHours2结束时间为空时为0
   {
...
Dygraph.prototype.toPercentXCoord = function (x) {
  ...
  if (logscale === true) {
    ...
    // 只考虑了一天中午休市  待修改
    // let timeVal = new Date(x).getHours()*60 + new Date(x).getMinutes();
    // 距离最新日期几天
    // tradingHours不为空的
    if(tradingHours1[2]!=0 && tradingHours1[3]!=0 ){
      ...
      {
        let day = Math.floor((x - xRange[0])/24/60/60/1000);
        // 总天数
        let dayAll = Math.floor((xRange[1] - xRange[0])/24/60/60/1000);
        // 休市时间
        let breaktime = 0
        ...
        // 闭市时间 到第二日
        let stoptime = 24*60*60*1000 - (tradingHours2[1].getTime() - tradingHours1[0].getTime());
        if(stoptime <= 0)
        {
          stoptime = 0;
        }
        let scale = xRange[1] - xRange[0];
        // 计算最大最小日期实际间隔
        let limitStart = new Date(xRange[0]).getHours()*60 + new Date(xRange[0]).getMinutes();
        let limitEnd = new Date(xRange[1]).getHours()*60 + new Date(xRange[1]).getMinutes();
