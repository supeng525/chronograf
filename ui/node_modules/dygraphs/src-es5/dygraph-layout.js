...
DygraphLayout.prototype._evaluateLimits = function () {
  ...
  var stoptime = 0;
  //  // 计算最大最小日期实际间隔
  //  var limitStart = new Date(xlimits[0]).getHours()*60 + new Date(xlimits[0]).getMinutes();
  //  var limitEnd = new Date(xlimits[1]).getHours()*60 + new Date(xlimits[1]).getMinutes();
  //  // 标记交易时段分类类型  0：为不设置交易时段 1：tradingHours1和2非空 day+1 2:day-1 3:day不变 4：tradingHours1非空  5：夜盘
  //  var type = 0; 
  //  // 最大最小日期休市时间  stoptime几天  
  //  day = Math.floor((new Date(new Date(xlimits[1]).toLocaleDateString()).getTime() 
  //  - new Date(new Date(xlimits[0]).toLocaleDateString()).getTime())/24/60/60/1000)
  //  // console.log("test1",limitStart,limitEnd,day,(xlimits[1] - xlimits[0])/24/60/60/1000)
  ...
  //       xrange = xrange - (breaktime*(day+1) + (stoptime*day));
  ...
  //       type = 2;
  //     }else{
  //       xrange = xrange - (breaktime + stoptime)*day;
  ...
  //       {
  //         xrange = xrange - stoptime*(day-1);
  //       }else{
  //         xrange = xrange - stoptime*(day-2);
  //       }
  //     }
  //     // 夜盘
  //     type = 5;
    ...
    switch(xAxis.type){
      ...
      case 5: // tradingHours1非空 夜盘
          ...
          // 最大时间 >=夜盘开始时间
            if(timeVal >= xAxis.tradingHours2[2])
            {
              temp = value - xAxis.stoptime*(day-1);
            }else{
              temp = value - xAxis.stoptime*(day-2);
            }
          }
          break;
      default:
